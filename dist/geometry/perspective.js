import { computeHomography, invertHomography, transformPoint } from './homography.js';
...
export function warpPerspective(source, sourceToDestination, width, height) {
    ...
    for (let y = 0; y < height; y++)
        for (let x = 0; x < width; x++) {
            ...
            for (let c = 0; c < 4; c++)
                out[index + c] = sample(source, sourcePoint.x, sourcePoint.y, c);
        }
    return new ImageData(out, width, height);
}
export function rectifyPerspective(source, corners, width = source.width, height = source.height) {
    const rectangle = [{ x: 0, y: 0 }, { x: width - 1, y: 0 }, { x: width - 1, y: height - 1 }, { x: 0, y: height - 1 }];
