import { imageDataToBlob, loadImage, type BrowserImageInput } from '../browser/image.js';
...
async function jpegAttack(image: ImageData, quality: number): Promise<ImageData> {
  const blob = await imageDataToBlob(image, 'image/jpeg', quality);
  return (await loadImage(blob)).imageData;
}

export async function analyzeRobustness(input: BrowserImageInput, options: AnalyzeRobustnessOptions): Promise<RobustnessReport> {
  if (!options.payload && !options.fingerprint) throw new Error('Provide payload or fingerprint.');
  ...
  const cases: Array<[string, ImageData, number[]?, number?]> = [
    ['original', embedded, [8]],
  ...
  if (options.includeJpeg ?? true) {
    for (const quality of [0.90, 0.75, 0.60, 0.45]) cases.push([`jpeg${Math.round(quality * 100)}`, await jpegAttack(embedded, quality), [8]]);
  }
