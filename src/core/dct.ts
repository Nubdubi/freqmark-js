...
const COS = Array.from({ length: BLOCK_SIZE }, (_, x) =>
  Array.from({ length: BLOCK_SIZE }, (_, u) =>
    Math.cos(((2 * x + 1) * u * Math.PI) / 16),
  ),
);

const C = Array.from({ length: BLOCK_SIZE }, (_, i) =>
  i === 0 ? 1 / Math.sqrt(2) : 1,
);

export type Block8 = number[][];

export function dct8(block: Block8): Block8 {
  if (block.length !== BLOCK_SIZE || block.some((row) => row.length !== BLOCK_SIZE)) {
    throw new Error('dct8 expects an 8x8 block.');
  }

  const out: Block8 = Array.from({ length: BLOCK_SIZE }, () =>
...
export function idct8(coeff: Block8): Block8 {
