 ...
 */
export function embedSignedQim(value: number, bit: WatermarkBit, step: number): number {
  if (!(step > 0) || !Number.isFinite(step)) throw new Error('QIM step must be positive.');
  const wantedSign = bit === 1 ? 1 : -1;
  const half = step / 2;
  if (value * wantedSign <= 0) return wantedSign * half;
  const magnitude = Math.abs(value);
  const bucket = Math.max(0, Math.round((magnitude - half) / step));
  return wantedSign * (bucket * step + half);
}

export function readSignedQim(value: number): WatermarkBit {
  return value >= 0 ? 1 : 0;
}
