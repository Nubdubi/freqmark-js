...
export class RepetitionCodec implements EccCodec {
  ...
  encode(data: Uint8Array): Uint8Array {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let r = 0; r < this.repetitions; r++) out[r * data.length + i] = data[i] & 1;
    }
    return out;
  }
  ...
  decode(data: Uint8Array): EccDecodeResult {
    ...
    for (let i = 0; i < out.length; i++) {
      let ones = 0;
      for (let r = 0; r < this.repetitions; r++) ones += data[r * out.length + i] & 1;
