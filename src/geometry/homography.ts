...
function solveLinearSystem(matrix: number[][], values: number[]): Float64Array {
  ...
  for (let column = 0; column < n; column++) {
    ...
    if (Math.abs(augmented[pivot][column]) < 1e-10) throw new Error('Degenerate homography points.');
...
export function computeHomography(source: Quad, destination: Quad): Homography {
  const matrix: number[][] = [];
  const values: number[] = [];
  for (let i = 0; i < 4; i++) {
    const { x, y } = source[i];
...
export function transformPoint(h: Homography, point: Point2D): Point2D {
