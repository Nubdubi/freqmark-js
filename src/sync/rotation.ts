...
import { detectFingerprintFromImageData, type DetectEngineOptions, type DetectEngineResult } from '../watermark/engine.js';
...
function candidateScore(result: DetectEngineResult): number {
  return (result.found ? 2 : 0) + result.confidence + result.sync.confidence * 0.25;
}

export function searchRotationFromImageData(
  source: ImageData,
  detectOptions: DetectEngineOptions,
  searchOptions: RotationSearchOptions = {},
): DetectEngineResult {
  ...
  let best: DetectEngineResult | null = null;
  let bestAngle = 0;
  let totalCandidates = 0;
  const evaluatedAngles = new Set<number>();
  const evaluate = (angle: number): void => {
    const key = Math.round(angle * 1000) / 1000;
    if (evaluatedAngles.has(key)) return;
  ...
  for (const angle of coarse) evaluate(angle);
  if (!explicit?.length) {
    const coarseBest = best as DetectEngineResult | null;
    if (coarseBest?.found) {
      for (let angle = bestAngle - coarseStep; angle <= bestAngle + coarseStep + 1e-9; angle += fineStep) {
        if (angle >= -max && angle <= max) evaluate(angle);
      }
    } else {
      ...
      for (let angle = -max; angle <= max + 1e-9; angle += fineStep) evaluate(angle);
    }
  }
